...
class M_publikasi_dosen extends MY_Model {
	...
	*/
	...
	public function report_by_year($year=false){
		...
		$this->db->select("ISNULL(YEAR(pub_created_at), $year) as year", FALSE);
		...
		$this->db->join($this->table_name, "month=Month(pub_created_at) AND $year = YEAR(pub_created_at)", "left");
		$this->db->group_by(array("month", "Year(pub_created_at)"));
		$query = $this->db->get();
		return $query->result();
	}
	...
	{
		$this->db->select("YEAR(pub_created_at) as year", FALSE);
		$this->db->select("COUNT(pub_id) as jumlah", FALSE);
		...
		$this->db->group_by("YEAR(pub_created_at)");
		$query = $this->db->get();
		return $query->result();
	}
	public function report_by_unit($fakultas=false){
