 ...
 */
class M_nextcron extends MY_Model {
	protected $pk_col = 'nxc_id';
	protected $table_name = 'nextcron';
	...
	*/
	public function insert($nxc_user=false,
					$nxc_start=false,
					$nxc_end=false){
		$data = array();
        if($nxc_user       !== false)$data['nxc_user']       =trim($nxc_user);
        if($nxc_start      !== false)$data['nxc_start']      =trim($nxc_start);
        if($nxc_end     !== false)$data['nxc_end']     =trim($nxc_end);

		$this->db->insert('nextcron', $data);
	...
	* @param type $acc_delete      adalah field untuk kolom acc_delete     , default = false
	*/
	public function update($nxc_id=false,
					$nxc_user=false,
					$nxc_start=false,
					$nxc_end=false){
		$data = array();
		...
		return $this->db->update('nextcron', $data, "nxc_id = '$nxc_id'");
	}
