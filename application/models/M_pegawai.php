...
class M_pegawai extends MY_Model {
	...
	*/
	...
	public function get_url_kosong(){
		$this->db->distinct();
		$this->db->select("peg_jurusan");
		$this->db->select("jur_nama_indonesia");
		$this->db->select("peg_nip_baru");
		...
		$this->db->join('jurusan', 'peg_jurusan = jur_id', 'left');
		$this->db->where("(peg_google_schoolar = '' OR peg_google_schoolar is NULL) AND (peg_status_aktivitas_pegawai in ('A','DP','TB','TI','DK','DT','MP') and peg_is_dosen=1 and peg_ikatan_kerja_pegawai in (1,2,3) and peg_deleted_at is NULL)");
