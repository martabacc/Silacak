...
/*
 * Controller for handling detil_kode_publikasi module
 * @author : Doni Setio Pambudi (donisp06@gmail.com)
 */
class Detil_kode_publikasi extends CI_Controller {
	...
	public function __construct() {
		...
		//$this->auth->set_default_module('detil_kode_publikasi');
	...
	/*
	 * method index controller detil_kode_publikasi
	...
	public function index(){
		...
		//add menu highlight
		$this->site_info->set_current_module('detil_kode_publikasi');

		//add masterpage script
		$this->asset_library->add_masterpage_script();
		//add page javascript
		$this->asset_library->add_js('js/pages/detil_kode_publikasi.js');
		...
		$this->load->view('detil_kode_publikasi/master', $data);
	...
	public function get_datamaster(){
		...
		//get data
		$this->m_detil_kode_publikasi->get_datatable($where);
	}
	...
	public function detail(){
		...
		$search_data = $this->input->post('dkp_id');
		...
		//get detail
		$detail = $this->m_detil_kode_publikasi->get_by_column($search_data);
	...
	private function save(){
		...
		$this->form_validation->set_rules('dkp_id'        , 'lang:dkp_id'        , 'integer');
		$this->form_validation->set_rules('dkp_kode'      , 'lang:dkp_kode'      , 'trim|max_length[2]');
		$this->form_validation->set_rules('dkp_keterangan', 'lang:dkp_keterangan', 'trim|max_length[255]');

		if ($this->form_validation->run())
		{
			// insert
			if($this->input->post('dkp_id') == '')
			{
				$dkp_id = $this->m_detil_kode_publikasi->insert(
					$this->input->post('dkp_kode'),
					$this->input->post('dkp_keterangan')
				);
				write_log('detil_kode_publikasi', 'insert', "PK = $dkp_id");
				ajax_response();
			}
			// update
			else
			{
				$pk_id = $this->input->post('dkp_id');
				//check if pk integer
				//$pk_id = uintval($pk_id);
				$this->m_detil_kode_publikasi->update(
					$pk_id,
					$this->input->post('dkp_kode'),
					$this->input->post('dkp_keterangan')
				);
				write_log('detil_kode_publikasi', 'update', "PK = $pk_id");
				ajax_response();
			}
		}
		else
	...
	public function delete(){
		...
		if($this->input->post('dkp_id') === NULL) ajax_response();

		$all_deleted = array();
		foreach($this->input->post('dkp_id') as $row){
			//$row = uintval($row);
			//permanent delete row, check MY_Model, you can set flag with ->update_single_column
			$this->m_detil_kode_publikasi->permanent_delete($row);

			//this is sample code if you cannot delete, but you must update status
			//$this->m_detil_kode_publikasi->update_single_column('dkp_deleted', 1, $row);
			$all_deleted[] = $row;
		}
		write_log('detil_kode_publikasi', 'delete', 'PK = ' . implode(",", $all_deleted));
