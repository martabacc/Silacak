...
class Publikasi_dosen extends CI_Controller {
	...
	public function ind_dbg(){
		...
		$this->site_info->add_breadcrumb($this->lang->line('module_title'));
		//add menu highlight
		$this->site_info->set_current_module('publikasi_dosen_debug');
		...
		// added for lab filter
		$this->load->model('m_repository_simpel');
		$data['filter_lab'] = $this->m_repository_simpel->getListLab();

		//added scripts and stylesheet for select2
		$this->asset_library->add_css('css/select2.min.css');
		$this->asset_library->add_js('js/select2.min.js');

		//load view
		$this->load->view('base/header');
		$this->load->view('publikasi_dosen/master2', $data);
		$this->load->view('base/footer');
	}
	...
	public function get_datamaster(){
		...
		$filter_cols = array('pub_detilkodepub' => /*uintval(*/$this->input->post('pub_detilkodepub'),  
							 'pub_status_tarik' => $this->input->post('pub_status_tarik')/*, TRUE)*/);
	...
	public function get_datamaster_debug(){
		//only ajax is allowed
		if(!$this->input->is_ajax_request()) show_404();
		...
		//if the lab filter is used - build masterpage filter cannot be used
